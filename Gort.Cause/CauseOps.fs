...
module CauseOps =
    ...
    let GetAllCausesForWorkspace (ctxt: Gort.Data.DataModel.IGortContext) (wsName: String) =
        try
            CauseQuery.GetAllCausesForWorkspace(wsName, ctxt) |> Ok
        with ex ->
            ("error in GetAllCausesForWorkspace: " + ex.Message) |> Result.Error


    let GetPendingCauseForWorkspace (ctxt: Gort.Data.DataModel.IGortContext) (wsName: String) =
        try
            CauseQuery.GetPendingCauseForWorkspace(wsName, ctxt) |> Ok
        with ex ->
            ("error in GetAllCausesForWorkspace: " + ex.Message) |> Result.Error
    ...
    let GetCauseById (ctxt: Gort.Data.DataModel.IGortContext) (causeId: Guid) =
        try
            CauseQuery.GetCauseById(causeId, ctxt) |> Ok
        with ex ->
            ("error in GetCauseById: " + ex.Message) |> Result.Error


    let GetCauseTypeGroupAncestry (ctxt: Gort.Data.DataModel.IGortContext) (cauz: Gort.Data.DataModel.Cause) =
        try
            CauseQuery.GetCauseTypeGroupAncestry(cauz, ctxt) |> Ok
        with ex ->
            ("error in GetCauseTypeGroupAncestry: " + ex.Message) |> Result.Error


    let GetParamType (cauz: Gort.Data.DataModel.ParamTypeName) (ctxt: Gort.Data.DataModel.IGortContext) =
        try
            CauseQuery.GetParamType(cauz, ctxt) |> Ok
        with ex ->
            ("error in GetParamType: " + ex.Message) |> Result.Error
