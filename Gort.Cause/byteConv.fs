...
module byteConv =
    ...
    let intFromBytes (bVals: byte[]) =
        ...
        with ex ->
            ("error in getInt: " + ex.Message) |> Result.Error
    ...
    let intArrayFromBytes (bVals: byte[]) =
        try
            ...
            iB |> Ok
        with ex ->
            ("error in getIntArray: " + ex.Message) |> Result.Error
    ...
    let doubleFromBytes (bVals: byte[]) =
        ...
        with ex ->
            ("error in getDouble: " + ex.Message) |> Result.Error
    ...
    let guidFromBytes (bVals: byte[]) =
        try
            Guid(bVals) |> Ok
        with ex ->
            ("error in getGuid: " + ex.Message) |> Result.Error
    ...
    let guidArrayFromBytes (bVals: byte[]) =
        try
            ...
            gA |> Ok
        with ex ->
            ("error in guidArrayFromBytes: " + ex.Message) |> Result.Error
    ...
    let bytesFromInt (intV: int) =
        ...
        with ex ->
            ("error in bytesFromInt: " + ex.Message) |> Result.Error
    ...
    let bytesFromIntArray (intA: int[]) =
        try
            ...
            iB |> Ok
        with ex ->
            ("error in bytesFromIntArray: " + ex.Message) |> Result.Error
    ...
    let bytesFromDouble (fltVal: float) =
        ...
        with ex ->
            ("error in bytesFromDouble: " + ex.Message) |> Result.Error
    ...
    let bytesFromDoubleAray (dblA: float[]) =
        try
            ...
            dB |> Ok
        with ex ->
            ("error in bytesFromDoubleAray: " + ex.Message) |> Result.Error
    ...
    let bytesFromString (str: string) =
        ...
        with ex ->
            ("error in bytesFromString: " + ex.Message) |> Result.Error
    ...
    let bytesFromStringArray (strA: string[]) =
        ...
        with ex ->
            ("error in bytesFromStringArray: " + ex.Message) |> Result.Error
    ...
    let bytesFromGuid (guVal: Guid) =
        ...
        with ex ->
            ("error in bytesFromGuid: " + ex.Message) |> Result.Error
    ...
    let bytesFromGuidArray (guids: Guid[]) =
        try
            ...
            gA |> Ok
        with ex ->
            ("error in bytesFromGuidArray: " + ex.Message) |> Result.Error
