...
module ctrUtils =
    ...
    let runMakeRandGenRecord (cz: Cause) (ctxt: Gort.Data.DataModel.IGortContext) =
        result {
            let prams = cz.CauseParams |> Seq.toList

            let pramRandGenType =
                prams
                |> List.find (fun p -> p.CauseParamType.Name = CauseParamTypeName.RandGenType.ToString())

            let pramRandGenSeed =
                prams
                |> List.find (fun p -> p.CauseParamType.Name = CauseParamTypeName.RandGenSeed.ToString())
    ...
    let runMakeSetOfRandGenRecords (cz: Cause) (ctxt: Gort.Data.DataModel.IGortContext) =
        result {
            let prams = cz.CauseParams |> Seq.toList
            ...
            let pramCount =
                prams
                |> List.find (fun p -> p.CauseParamType.Name = CauseParamTypeName.RandGenCount.ToString())
