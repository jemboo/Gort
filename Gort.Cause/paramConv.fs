...
module paramConv =
    ...
    let MutationTypeFromParam (pram:Param) =
        result {
            if (pram.ParamType.Name <> ParamTypeName.MutationType.ToString()) then
               let errMsg = sprintf "%s is not %s" 
                                pram.ParamType.Name 
                                (ParamTypeName.MutationType.ToString())
               return! errMsg |> Error
            else
    ...
    let NumberFormatFromParam (pram:Param) =
        result {
            if (pram.ParamType.Name <> ParamTypeName.NumberFormat.ToString()) then
               let errMsg = sprintf "%s is not %s" 
                                pram.ParamType.Name 
                                (ParamTypeName.NumberFormat.ToString())
               return! errMsg |> Error
            else
              let! rtv = pram.Value |> byteConv.intFromBytes
    ...
    let RandGenTypeFromParam (pram:Param) =
        result {
            if (pram.ParamType.Name <> ParamTypeName.RandGenType.ToString()) then
               let errMsg = sprintf "%s is not %s" 
                                pram.ParamType.Name 
                                (ParamTypeName.RandGenType.ToString())
               return! errMsg |> Error
            else
              let! rtv = pram.Value |> byteConv.intFromBytes
    ...
    let SortableFormatFromParam (pram:Param) =
        result {
            if (pram.ParamType.Name <> ParamTypeName.SortableFormat.ToString()) then
               let errMsg = sprintf "%s is not %s" 
                                pram.ParamType.Name 
                                (ParamTypeName.SortableFormat.ToString())
               return! errMsg |> Error
            else
              let! rtv = pram.Value |> byteConv.intFromBytes
    ...
    let SortableSetRepFromParam (pram:Param) =
        result {
            if (pram.ParamType.Name <> ParamTypeName.SortableSetRep.ToString()) then
               let errMsg = sprintf "%s is not %s" 
                                pram.ParamType.Name 
                                (ParamTypeName.SortableSetRep.ToString())
               return! errMsg |> Error
            else
              let! rtv = pram.Value |> byteConv.intFromBytes
    ...
    let SorterSaveModeFromParam (pram:Param) =
        result {
            if (pram.ParamType.Name <> ParamTypeName.SorterSaveMode.ToString()) then
               let errMsg = sprintf "%s is not %s" 
                                pram.ParamType.Name 
                                (ParamTypeName.SorterSaveMode.ToString())
               return! errMsg |> Error
            else
              let! rtv = pram.Value |> byteConv.intFromBytes
    ...
    let RandoFromRandGenRecordParam (ctxt:Gort.Data.DataModel.IGortContext)
                                    (pram:Param) =
        result {
            if (pram.ParamType.Name <> ParamTypeName.RandGenId.ToString()) then
               let errMsg = sprintf "%s is not %s" 
                                pram.ParamType.Name 
                                (ParamTypeName.RandGenId.ToString())
               return! errMsg |> Error
            else
