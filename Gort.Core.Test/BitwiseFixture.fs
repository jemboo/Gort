...
type BitwiseFixture () =
    ...
    [<TestMethod>]
    member this.bitPackedtoBitStriped2 () =
        let deg = Degree.create 8
        let bpa = Bitwise.allBitPackForDegree deg |> Result.ExtractOrThrow
        let bsa = bpa |> Bitwise.bitPackedtoBitStriped2 deg |> Result.ExtractOrThrow
        let bpaBack = bsa |> Bitwise.bitStripedToBitPacked2 deg bpa.Length |> Result.ExtractOrThrow
        Assert.AreEqual(bpa |> Array.toList, bpaBack |> Array.toList)

        let deg2 = Degree.create 16
        let randy = Rando.fromRngGen (RngGen.lcgFromNow ())
        let bpa2 = Array.init 100 (fun _ -> RndGen.rndBitsUint64 deg2 randy)
        let bsa2 = bpa2 |> Bitwise.bitPackedtoBitStriped2 deg2 |> Result.ExtractOrThrow
        let bpaBack2 = bsa2 |> Bitwise.bitStripedToBitPacked2 deg2 bpa2.Length |> Result.ExtractOrThrow
