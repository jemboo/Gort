...
module SortableBySwitchTrackerStandard = 
    ...
    let getSortableBySwitchArrays 
            (sortableBySwitchTrackerStandard:sortableBySwitchTrackerStandard) =
        let arrayLength = sortableBySwitchTrackerStandard 
                          |> getArrayLength
        sortableBySwitchTrackerStandard.useRoll
        |> BooleanRoll.getData
    ...
    let getSwitchUseCounts (sortableBySwitchTrackerStandard:sortableBySwitchTrackerStandard) =
        let switchCt = sortableBySwitchTrackerStandard 
                            |> getSwitchCount
                            |> SwitchCount.value
        let useFlags = sortableBySwitchTrackerStandard 
                       |> getData
        ...
        while (flagDex < (useFlags.Length)) do
            if useFlags.[flagDex] then
                switchUseCts.[flagDex % switchCt] <-
                    switchUseCts.[flagDex % switchCt] + 1
            flagDex <- flagDex + 1
        switchUseCts
...
module SortableBySwitchTrackerBitStriped =
    ...
    let create (switchCount:switchCount) 
        ...
        let useRoll = Bs64Roll.createEmptyStripedSet 
                            arrayLength
                            arrayCount
    ...
    let getData (sbstbs:sortableBySwitchTrackerBitStriped) =
        sbstbs.useRoll |> Bs64Roll.getData

    let getSortableBySwitchArrays (sbstbs:sortableBySwitchTrackerBitStriped) =
        sbstbs.useRoll |> Bs64Roll.toBoolArrays
    ...
    let getSwitchUseCounts (sbstbs:sortableBySwitchTrackerBitStriped) =
        ...
        let _procChunk (useFlags:bool[]) =
            ...
            while (flagDex < useFlags.Length) do
                if useFlags.[flagDex] then
                    switchUseCts.[flagDex] <-
                        switchUseCts.[flagDex] + 1
                flagDex <- flagDex + 1

        let ignoreMe =
            sbstbs 
            |> getSortableBySwitchArrays
            |> Seq.iter _procChunk
...
module SortableBySwitchTracker =
    ...
    let getSortableBySwitchArrays (sortableBySwitchTracker:sortableBySwitchTracker) = 
        ...
        | ArrayRoll switchBySortableTrackerStandard ->
            switchBySortableTrackerStandard |> SortableBySwitchTrackerStandard.getSortableBySwitchArrays
        | BitStriped switchBySortableTrackerBitStriped -> 
            switchBySortableTrackerBitStriped |> SortableBySwitchTrackerBitStriped.getSortableBySwitchArrays


    let getSortableCount (sortableBySwitchTracker:sortableBySwitchTracker) =
        match sortableBySwitchTracker with
        | ArrayRoll switchBySortableTrackerStandard ->
            switchBySortableTrackerStandard |> SortableBySwitchTrackerStandard.getSortableCount
        | BitStriped switchBySortableTrackerBitStriped -> 
            switchBySortableTrackerBitStriped |> SortableBySwitchTrackerBitStriped.getSortableCount


    let getSwitchCount (sortableBySwitchTracker:sortableBySwitchTracker) =
        match sortableBySwitchTracker with
        | ArrayRoll switchBySortableTrackerStandard ->
            switchBySortableTrackerStandard |> SortableBySwitchTrackerStandard.getSwitchCount
        | BitStriped switchBySortableTrackerBitStriped -> 
            switchBySortableTrackerBitStriped |> SortableBySwitchTrackerBitStriped.getSwitchCount


    let getSwitchUseCounts (sortableBySwitchTracker:sortableBySwitchTracker) =
        match sortableBySwitchTracker with
        | ArrayRoll switchBySortableTrackerStandard ->
            switchBySortableTrackerStandard |> SortableBySwitchTrackerStandard.getSwitchUseCounts
        | BitStriped switchBySortableTrackerBitStriped -> 
            switchBySortableTrackerBitStriped |> SortableBySwitchTrackerBitStriped.getSwitchUseCounts
