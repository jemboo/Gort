...
module SwitchUseTrackerStandard =
   ...
   let getUseCounters (switchUses:switchUseTrackerStandard) =
        switchUses.useCounts

   let make (switchCount:switchCount) =
       { switchUseTrackerStandard.useCounts = 
...
module SwitchUseTracker =
   ...
   let init (switchOpMod:switchOpMode) (switchCount:switchCount) =
       ...
       | switchOpMode.Standard ->
            switchCount
            |> SwitchUseTrackerStandard.make
            |> switchUseTracker.Standard
       | switchOpMode.BitStriped ->
            SwitchUseTrackerBitStriped.make switchCount
            |> switchUseTracker.BitStriped


   let getUseFlags (switchUses:switchUseTrackerBitStriped) =
        switchUses.useFlags
   ...
   let getSwitchUseCounts (switchUs:switchUseTracker) =
       ...
       | switchUseTracker.Standard switchUseTrackerStandard -> 
            ...
            |> SwitchUseTrackerStandard.getUseCounters
       | switchUseTracker.BitStriped switchUseTrackerBitStriped -> 
            switchUseTrackerBitStriped 
            |> SwitchUseTrackerBitStriped.getSwitchUseCounts
