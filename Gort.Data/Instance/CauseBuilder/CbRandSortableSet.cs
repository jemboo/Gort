...
namespace Gort.Data.Instance.CauseBuilder
{
    ...
    public class CbRandSortableSet : CauseBuilderBase
    {
        public CbRandSortableSet(
            ...
            string descr, Param paramOrder, Param paramSortableCount,
            Param paramSortableFormat, Param paramRngId) : base(workspaceName, causeIndex)
        {
            PramOrder = paramOrder;
            PramRngId = MakeParam(ParamTypes.RecordId, RngId);
            PramSortableCount = paramSortableCount;
            PramSortableFormat = paramSortableFormat;

            Order = paramOrder.IntValue();
            SortableCount = paramSortableCount.IntValue();
            RngId = paramRngId.GuidValue();
            SortableFormat = paramSortableFormat.SortableFormatValue();
            CauseDescription = $"SortableSetRand({descr},{Order},{SortableCount})";

            CauseMakeSortableSet = MakeCause(CauseTypes.SortableSetRand);
            CauseParam_MakeSortableSet_RngId = MakeCauseParam(CauseParamTypes.SortableSetRand_RngId, CauseMakeSortableSet, PramRngId);
            CauseParam_MakeSortableSet_Order = MakeCauseParam(CauseParamTypes.SortableSetRand_Order, CauseMakeSortableSet, PramOrder);
            CauseParam_MakeSortableSet_SortableCount = MakeCauseParam(CauseParamTypes.SortableSetRand_SortableCount, CauseMakeSortableSet, PramSortableCount);
            CauseParam_MakeSortableSet_SortableFormat = MakeCauseParam(CauseParamTypes.SortableSetRand_SortableFormat, CauseMakeSortableSet, PramSortableFormat);
        }
