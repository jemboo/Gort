...
type System.UInt64 with                        // x=this
    ...
    member x.rev =                              // reverse bits
      let rec go b i acc =
        if i = 64 then acc else go (b>>>1) (i+1) ((acc <<< 1) ||| (b &&& 1uL))
      go x 0 0uL
    member x.count =                            // count bits set to 1
      let rec go b acc = if b = 0uL then acc else go (b &&& (b-1uL)) (acc+1uL) //sparse count
      //if b = 0 then acc else go (b.shrl) (acc + (b &&& 1)) //add res of calc
      go x 0uL
    member x.count_dense = // The loop will execute once for each unset bit
    ...
    member x.leftmost_index = 
      if 0uL=x then 0
          ...
          let mutable r = 0
          let mutable y = x
          if y &&& 18446744069414584320UL <> 0uL then y <- y >>> 32; r <- r + 32
          if y &&& 4294901760UL <> 0uL then y <- y >>> 16; r <- r + 16
          if y &&& 65280UL <> 0uL then y <- y >>> 8; r <- r + 8
          if y &&& 240UL <> 0uL then y <- y >>> 4; r <- r + 4
          if y &&& 12UL <> 0uL then y <- y >>> 2; r <- r + 2
          if y &&& 2UL <> 0uL then r <- r + 1
          r
