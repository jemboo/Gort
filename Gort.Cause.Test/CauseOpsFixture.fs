...
type CauseOpsFixture() =
    ...
    member this.GetAllCausesForWorkspace() =
        let czs =
            CauseOps.GetAllCausesForWorkspace texto "WorkspaceRand" |> Result.ExtractOrThrow
    ...
    member this.GetCauseById() =
        let czs = CauseOps.GetCauseById texto gu |> Result.ExtractOrThrow
        Assert.IsTrue(czs.Index > 0)


    [<TestMethod>]
    member this.GetCauseTypeGroupAncestry() =
        let cz = CauseOps.GetCauseById texto gu |> Result.ExtractOrThrow
    ...
    member this.GetCauseDispatcherInfo() =
        ...
        let cz, cestry = CauseOps.GetCauseDispatcherInfo texto gu |> Result.ExtractOrThrow
        Assert.IsTrue(cestry.Length > 0)
